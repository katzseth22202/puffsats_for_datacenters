...
Distance does not hide a data center, but it buys warning time (\autoref{tab:data_center_sites}). Reaching a Lagrange point takes \qty{11.0}{\kilo\meter\per\second} against \qty{3.4}{\kilo\meter\per\second} to reach \qty{650}{\kilo\meter}~\cite{katz2026aim}. Even at \qty{14}{\kilo\meter\per\second}, an interceptor needs half a day to reach Earth--Moon L4/L5 and two days to reach Sun--Earth L1. That is time to disperse a cluster or launch a defense, though no nation yet has one in space.

Latency decides which workloads go where. High Earth orbit at \qtyrange{20000}{80000}{\kilo\meter} adds \qtyrange{0.13}{0.53}{\second} of round-trip delay and still gives \qty{0.5}{\hour} to \qty{14}{\hour} of warning, so it suits latency-sensitive inference. Everything else can go to the Lagrange points. Their \qtyrange{2.6}{10}{\second} round trip is far inside the \qty{24}{\hour} turnaround that batch inference services already offer~\cite{anthropic_message_batches,openai_batch_api}. Sun--Earth L1 has no eclipses~\cite{nasa_lagrange_points}. The cost is exposure to solar particle events outside the magnetosphere~\cite{li2019vanallen}. Suncatcher's proton test ran a commercial tensor processing unit to 15~krad(Si) without hard failures~\cite{aguera2026_suncatcher}.

Starship~\cite{starship} can reach these sites, but probably not at the price orbital computing needs. Starcloud's chief executive puts that price near \$500 per \si{\kilo\gram}~\cite{bennici2026_johnston}. SpaceX's IPO underwriters, who have reason to be optimistic~\cite{michaely1999_underwriter}, forecast \$183 (Goldman Sachs~\cite{pethokoukis2026_goldman}) and about \$500 (Morgan Stanley~\cite{investing2026_ms_spacex}) per \si{\kilo\gram} to low Earth orbit in 2030. Sun--Earth L1 lies a further \qtyrange{3.2}{4.0}{\kilo\meter\per\second} out, paid for with propellant from two to four tanker flights. Each kilogram delivered there costs 3.5 to 4.6 kilograms launched to low Earth orbit~\cite{katz2026aim}, turning the forecasts into \$640--840 and \$1750--2300 per \si{\kilo\gram}. These are floors. They omit orbital propellant transfer, which SpaceX lists as a material challenge~\cite{spacex2026_s1a}. They also omit the return. A ship back from L1 enters at \qty{11.1}{\kilo\meter\per\second} rather than \qty{8.0}{\kilo\meter\per\second}, about 2.7 times the peak heating~\cite{katz2026aim}, on a heat shield SpaceX's chief executive called Starship's biggest challenge~\cite{musk2024_heatshield}. \autoref{sec:launch_cost_speculation} argues that PuffSat Pulsed Propulsion can meet the \$500 per \si{\kilo\gram} target at these sites.
