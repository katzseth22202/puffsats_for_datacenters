...
The rocket releases the PuffSats \qtyrange{1.1}{1.3}{\astronomicalunit} from the Sun. From \SI{1.3}{\astronomicalunit} the coast to Earth lasts about 16 days~\cite{katz2026aim}. Just before release, a deployment mechanism spins and orients each PuffSat and kneads its explosive emulsion. Kneading mixes in the sensitizers, which stay out until late in flight so that the charge is hard to set off by accident~\cite{maranda2025comprehensive}. It also mixes in slow-acting thickeners that lock the sensitizing bubbles in place, and it lets each PuffSat fly without a pump.
