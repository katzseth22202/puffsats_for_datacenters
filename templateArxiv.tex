...
\subsection{Deployment Rocket}
A large rocket carries the PuffSats around Jupiter, waiting until returning close to Earth to deploy them.  The large rocket can keep the PuffSats warm.   Juno showed solar panels can keep satellites warm at Jupiter, with a small battery for additional warmth for a few hours in Jupiter's shadow.  However, our gravity assist happens in the ecliptic plane where Jupiter's radiation is most severe.   Luckily, PuffSat payloads, mostly light volatiles, can serve as radiation shielding.   As long as electronics are shielding by \qtyrange{50}{100}{\centi\meter} of PuffSat mass, radiation should be minimal.  The PuffSat mass may want to contain a small amount of boron or another neutron poison for preventing neutron spallation from damaging the electronics.   Since PuffSats mass around Jupiter would be large compared to the science satellites we've sent past Jupiter in the past, the square-cubed law limits the relative mass of panels required to keep things warm.   The radiation may make non-imaging concentrators using mirrors or Fresnel lenses preferable to solar panels.   They can also shine light on a mirrored cavity that carries light to panels inside the craft, preventing energetic particles from hitting the solar panels directly.  
Between \qtyrange{1.1}{1.3}{\AU} the PuffSats are deployed.  For the most part the rocket can position each PuffSat with mechanical deployment.   The deployment mechanism spins and orients the PuffSats.   It also kneads the construction emulsion to evenly mix sensitizers and thickeners that lock the sensitized bubbles in place.  Each PuffSat is positions by the main rocket executing a very modest burn so that the ensemble arrives at Earth over a few minutes time.  
